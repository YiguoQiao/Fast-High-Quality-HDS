function bwimg1=PyraUpsampleClass(bwimg,bwimg1,Io1_1,de,win,wins,th,alf)
th1=5;
alf1=(2*wins*(de+1))^2; 
...
for i=-wins:wins
    for j=-wins:wins
        if ~(i==0 && j==0)
            ...
            Dis0=(sum((Ix-Ixx).^2,2)./alf+repmat((i^2+j^2)/alf1,size(lc))).*(bwx~=0);
            Dis0(Dis0==0)=Inf; 
            Dis(:,iter)=Dis0;
            BWX(:,iter)=bwx;
        end
...
while any(any(isnan(bwimg1)))
bwimg1=fillmissing(bwimg1,'movmedian',3);
end
