function Depth_Upper=PyraUpsampleClass(Depth_IM,Depth_Upper,RGB_Upper,win,wins,th_edge,th_class,th_mem,alpha,sigma_c,sigma_d)  
...
for i=-wins:wins
    for j=-wins:wins
        if ~(i==0 && j==0)
            ...
            Dis(Dis==0)=Inf; 
            Dis_c=(sum((RGB_index-RGB_index_neighbour).^2,2)./sigma_c).*(Depth_index_neighbour~=0);%color distance
            Dis_c(Dis_c==0)=Inf;
...
for i=1:size(Sorted_depth,1)
    ...
    W_c=Dis_c_all.^(-1);%weights according to color distance
